...
| 🚗 **[observability/vehicle-telemetry](observability/vehicle-telemetry)** | A full **observability vertical slice**: exporter → 10 alert rules with promtool unit tests → 14-panel Grafana dashboard → Compose *and* Kubernetes deploys → an AI agent that triages firing alerts. Ships a mock data source, so `--profile demo` runs the whole stack with live data and no hardware. |
